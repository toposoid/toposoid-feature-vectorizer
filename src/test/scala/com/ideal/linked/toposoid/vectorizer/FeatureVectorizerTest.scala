...
class FeatureVectorizerTest extends AnyFlatSpec with BeforeAndAfter with BeforeAndAfterAll{
  ...
  override def beforeAll(): Unit = {
    ToposoidUtils.callComponent("{}", conf.getString("TOPOSOID_WEAVIATE_ACCESSOR_HOST"), "9011", "createSchema")
  }
  ...
  override def afterAll(): Unit = {
    ...
    for (knowledgeForParser <- knowledgeForParsersJp) {
      ...
      ToposoidUtils.callComponent(Json.toJson(featureVectorIdentifier).toString(), conf.getString("TOPOSOID_WEAVIATE_ACCESSOR_HOST"), "9011", "delete")
    }
  ...
  "The list of japanese sentences" should "be properly registered in the vald and searchable." in {
    ...
    for ((knowledgeForParser, i) <- knowledgeForParsersJp.zipWithIndex) {
      ...
      val featureVectorSearchResultJson = ToposoidUtils.callComponent(searchJson, conf.getString("TOPOSOID_WEAVIATE_ACCESSOR_HOST"), "9011", "search")
